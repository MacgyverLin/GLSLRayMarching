...
public:
		...
		if (shaderToyDoc.HasMember("passes"))
		{
			...
			if (passesJson.IsArray())
			{
				...
				(
					...
					{ Pass::Filter::Linear, Pass::Filter::Nearest, Pass::Filter::Linear, Pass::Filter::Linear },	// input filter
				...
				(
					...
					{ "image", "easu", "rcas", "scaledimage" },														// input texture
					{ Pass::Filter::Linear, Pass::Filter::Linear, Pass::Filter::Linear, Pass::Filter::Linear },		// input filter
					{ Pass::Wrap::Clamp   , Pass::Wrap::Clamp   , Pass::Wrap::Clamp   , Pass::Wrap::Clamp },		// input wraps
					"copy.glsl",
					"default",
					commonShaderURL))
					return false;
	...
	bool CreatePostprocessPass(Pass& pass,
		const std::vector<std::string> channelBufferNames,
		...
		const char* shaderFileName, const char* renderTargetName, const char* commonShaderURL)
	{
		...
		for (int i = 0; i < CHANNEL_COUNT; i++)
		{
			...
			{
				pass.SetChannelFrameBuffer(i, GetFrameBuffer(channelBufferNames[i].c_str()));
			}
		}
	...
	Texture *LoadTexture2D(std::string& folder, rapidjson::Value& channelJson, int i, int j)
	{
		std::string url = folder;
		url += "/";
