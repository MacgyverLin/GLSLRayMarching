...
void DisplayCamera(FbxCamera* pCamera, char* pName, FbxNode* pTargetNode, FbxNode* pTargetUpNode)
{
    ...
    Debug("    Projection Type: ", lProjectionTypes[pCamera->ProjectionType.Get()]);
...
{
    Debug("    Camera Position and Orientation");
    Display3DVector("        Position: ", pCamera->Position.Get());

    if (pTargetNode)
    {
        Debug("        Camera Interest: ",(char *) pTargetNode->GetName());
    }
    else
    {
    ...
    if (pTargetUpNode)
    {
        Debug("        Camera Up Target: ", (char *) pTargetUpNode->GetName());
    }
    else
    {
...
void DisplayApertureAndFilmControls(FbxCamera* pCamera)
{
    Debug("    Aperture and Film Controls");
    ...
    Debug("        Aperture Format: ", lCameraApertureFormats[pCamera->GetApertureFormat()]);

    const char* lCameraApertureModes[] = { "Horizontal and Vertical", "Horizontal", "Vertical", "Focal Length" };

    Debug("        Aperture Mode: ", lCameraApertureModes[pCamera->GetApertureMode()]);
...
void DisplayBackgroundProperties(FbxCamera* pCamera)
{
    Debug("    Background Properties");

    Debug("        Background File Name: \"", (char *) pCamera->GetBackgroundFileName(), "\"");

    const char* lBackgroundDisplayModes[] = { "Disabled", "Always", "When Media" };

    Debug("        Background Display Mode: ", lBackgroundDisplayModes[pCamera->ViewFrustumBackPlaneMode.Get()]);
    ...
    if (!lBackgroundPlacementOptions.IsEmpty())
    {
        ...
        Debug("        Background Placement Options: ",lString.Buffer());
    }
    ...
    Debug("        Background Distance Mode: ", lCameraBackgroundDistanceModes[pCamera->BackPlaneDistanceMode.Get()]);
}


void DisplayCameraViewOptions(FbxCamera* pCamera)
{
    Debug("    Camera View Options");
    ...
    Debug("        Safe Area Style: ", lSafeAreaStyles[pCamera->SafeAreaDisplayStyle.Get()]);
...
void DisplayRenderOptions(FbxCamera* pCamera)
{
    ...
    Debug("        Render Options Usage Time: ", lCameraRenderOptionsUsageTimes[pCamera->UseRealTimeDOFAndAA.Get()]);
    ...
    Debug("        Antialiasing Method: ", lCameraAntialiasingMethods[pCamera->AntialiasingMethod.Get()]);
    ...
    Debug("        Sampling Type: ", lCameraSamplingTypes[pCamera->FrameSamplingType.Get()]);
    ...
    Debug("        Focus Distance Source: ", lCameraFocusDistanceSources[pCamera->FocusSource.Get()]);
...
void DisplayDefaultAnimationValues(FbxCamera* pCamera)
{
    Debug("    Default Animation Values");
