...
vec3 shadeplanet(vec3 pos, vec3 k) { 
	...
	// add some noise
	float noi=max(0.,texture(iChannel0,n.yz*.5).x-.6);
	noi+=max(0.,texture(iChannel0,n.yz).x-.6);
	noi+=max(0.,texture(iChannel0,n.yz*2.).x-.6);
	col+=noi*(1.5-c)*.7;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	if ((mod(iTime,20.)>15. && iMouse.z<1.)) {
		...
		//from.x+=.05;// from.y+=.5;
		vec2 fixpos=(texture(iChannel0,vec2(from.z*.21325)).xy-.5)*vec2(FOLD*2.-.3,1.);
