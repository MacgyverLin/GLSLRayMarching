...
bool IntersectLight(Light light, Ray ray, inout HitRecord hitRecord)
{
	float denom = dot(ray.dir, light.normal);
	float t = dot(light.position - ray.origin, light.normal) / denom;

	if (t > 0.0 && t < hitRecord.t_min)
	{
		vec3 p = RayAt(ray, t);

		bool hit = IsInside(p, light.position, light.normal, vec2(light.size * 0.5));
		if (hit)
		{
			hitRecord.hit = true;
			hitRecord.t_min = t;
			hitRecord.position = RayAt(ray, t);
			hitRecord.normal = normalize(light.normal);
