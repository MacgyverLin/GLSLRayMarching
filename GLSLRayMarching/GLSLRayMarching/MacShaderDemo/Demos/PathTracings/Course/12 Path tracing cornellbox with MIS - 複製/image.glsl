...
Light light = Light(0.5, 0.5 * 0.5, vec3(0.00, 0.90, 0.50), vec3(0, -1, 0), 0);
...
bool TraceRay(Ray ray, inout HitRecord hitRecord)
{
	...
	IntersectLight(light, ray, hitRecord);
		...
		else
		{
			...
			for (int i = 0; i < NUM_BOUNCES; i++)
			{
				// NEE
				{
					...
					vec3 y = SampleLight(light);
					vec3 ny = light.normal;
					...
					if (TraceShadowRay(hitRecord.position, y))
					{
						...
						if (G > 0.0)
						{
							...
							float lightPDF = 1.0 / (light.area * G);
