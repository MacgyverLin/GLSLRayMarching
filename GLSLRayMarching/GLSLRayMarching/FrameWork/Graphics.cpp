...
bool Graphics::Initialize(int width_, int height_, const char* appName_)
{
	...
	glGetIntegerv(GL_MAX_TEXTURE_UNITS, &impl->maxTextureUnits);
...
bool Graphics::PreUpdate()
{
#if (PLATFORM == GLFW)
	///////////////////////////////////////////////////
	// time
	double now = glfwGetTime();
	impl->deltaTime = now - impl->currentTime;
	impl->currentTime = now;
	...
	for (int i = 0; i < (int)Platform::KeyCode::Count; i++)
		impl->oldkeystates[i] = impl->keystates[i];
	...
	for (int i = 0; i < glfwKeyCodes.size(); i++)
	{
		...
		int keyState = glfwGetKey(impl->window, glfwKey);

		if (keyState == GLFW_PRESS)
		{
			impl->keystates[int(code)] = true;
	...
	for (int i = 0; i < glfwMouseKeyCodes.size(); i++)
	{
		...
		int keyState = glfwGetMouseButton(impl->window, glfwKey);
		if (keyState == GLFW_PRESS)
			impl->keystates[int(code)] = true;
		else if (keyState == GLFW_RELEASE)
			impl->keystates[int(code)] = false;
	...
	double newMouseX = 0;
	double newMouseY = 0;
	glfwGetCursorPos(impl->window, &newMouseX, &newMouseY);
	newMouseY = GetHeight() - 1 - newMouseY;
	impl->mouse.dx = (float)newMouseX - impl->mouse.x;
	impl->mouse.dy = (float)newMouseY - impl->mouse.y;
	impl->mouse.x = (float)newMouseX;
	impl->mouse.y = (float)newMouseY;
	...
	for (int i = GLFW_JOYSTICK_1; i <= GLFW_JOYSTICK_16 && i < JOYSTICK_COUNT; i++)
	{
		if (impl->joyStickConnected[i])
		{
			...
			{
				impl->joySticks[i].axis[j] = axes[j];
			...
			{
				impl->joySticks[i].button[j] = buttons[j];
...
const std::string& Graphics::GetAppName() const
{
	return impl->appName;
}
...
int Graphics::GetTotalFrameCounter() const
{
	return impl->totalFrameCounter;
}

int Graphics::GetSceneFrameCounter() const
{
	return impl->sceneFrameCounter;
}

void Graphics::ResetSceneFrameCounter()
{
	impl->sceneFrameCounter = -1;
}

int Graphics::GetKeyCount() const
{
	return (int)Platform::KeyCode::Count;
}

bool Graphics::GetKeyDown(Platform::KeyCode code) const
{
	return !impl->oldkeystates[(int)code] && impl->keystates[(int)code];
}

bool Graphics::GetKeyUp(Platform::KeyCode code) const
{
	return impl->oldkeystates[(int)code] && !impl->keystates[(int)code];
}

bool Graphics::GetKeyHold(Platform::KeyCode code) const
{
	return impl->oldkeystates[(int)code] && impl->keystates[(int)code];
}

bool Graphics::GetKey(Platform::KeyCode code) const
{
	return impl->keystates[(int)code];
}

Platform::Mouse Graphics::GetMouse() const
{
	return impl->mouse;
}

float Graphics::GetMouseX() const
{
	return impl->mouse.x;
}

float Graphics::GetMouseY() const
{
	return impl->mouse.y;
}

float Graphics::GetMouseDX() const
{
	return impl->mouse.dx;
}

float Graphics::GetMouseDY() const
{
	return impl->mouse.dy;
}

void Graphics::EnableCursor()
{
#if (PLATFORM == GLFW)
...
bool Graphics::IsCursorEnabled() const
{
#if (PLATFORM == GLFW)
	return glfwGetInputMode(impl->window, GLFW_CURSOR);
#elif (PLATFORM == MACOSX)
#elif (PLATFORM == LINUX)
#elif (PLATFORM == ANDROID)
#elif (PLATFORM == IOS)
...
bool Graphics::IsJoyStickConnected(int i) const
{
	return impl->joyStickConnected[i];
}

Platform::JoyStick Graphics::GetJoyStick(int i) const
{
	return impl->joySticks[i];
}

const std::vector<std::string>& Graphics::GetJoystickNames() const
{
	return impl->joyStickNames;
}

void Graphics::GetDropPaths(std::vector<std::string>& dropPaths_) const
{
	dropPaths_ = impl->dropPaths;
}

bool Graphics::hasDropPath() const
{
	return impl->dropPaths.size() != 0;
}

const char* Graphics::GetClipBoardString() const
{
	return glfwGetClipboardString(nullptr);
}

void Graphics::SetClipBoard(const char* s)
{
	glfwSetClipboardString(nullptr, s);
}

void Graphics::QuitApp()
{
